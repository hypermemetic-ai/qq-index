...
Cold scans still perform one authoritative, replay-validating `readSession` per listed session. Before projection, a narrow restart fast path accepts only the exact production `{ session, events }` envelope, the closed validated production header, and the complete validated daemon source-state shape. When the durable `nextSeq` exactly equals the raw event-array length and the absolute workspace identity plus its derived scope token exactly match durable state, the source counts the scan and performs no projection-helper calls or commits. Missing/behind/noncanonical cursors, generated or ambiguous shapes, and malformed or unverifiable headers/workspaces/events retain full projection. A cursor ahead of the raw length or an unchanged cursor with a workspace mismatch fails closed.
...
The rc.7 query/projection integration and locally installed grouped-query patch are transitional. The deployment is moving forward to released DSH `0.1.2-alpha.4`; do not treat rc.7 as the upgrade target or copy its patches forward. Revalidate the stock alpha.4 contracts and isolated UI/model/runtime gates during that coordinated upgrade.
